...
// ──────────────────────────────────────────────────────────────
// Query helpers para dashboard de auditoría
// ──────────────────────────────────────────────────────────────

export async function getRecentAuditEvents(limit = 50): Promise<any[]> {
...
export async function getAuditStats(sinceHours = 24): Promise<{
  totalExecutions: number;
  successfulExecutions: number;
  failedExecutions: number;
  avgDurationMs: number;
  topTools: Array<{ tool: string; count: number }>;
  groqUsageRate: number;
}> {
  const since = new Date(Date.now() - sinceHours * 60 * 60 * 1000);

  const [events, totalExecutions] = await Promise.all([
    db.agentAction.findMany({
      where: { type: "audit", createdAt: { gte: since } },
    }),
    db.agentAction.count({
      where: { type: "audit", title: { contains: "Tool ejecutada" }, createdAt: { gte: since } },
    }),
  ]);

  const successfulExecutions = events.filter(
    e => e.title.includes("✅") && e.title.includes("Tool ejecutada")
  ).length;
  ...
  for (const e of events) {
    try {
      const p = JSON.parse(e.payload || "{}");
      if (p.tool) toolCounts.set(p.tool, (toolCounts.get(p.tool) || 0) + 1);
    } catch {}
  }
  ...
  const groqEvents = events.filter(e => e.payload?.includes('"source":"groq"')).length;
  const groqUsageRate = events.length > 0 ? groqEvents / events.length : 0;

  return { totalExecutions, successfulExecutions, failedExecutions, avgDurationMs, topTools, groqUsageRate };
}
